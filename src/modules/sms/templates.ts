...
export const SYSTEM_SMS_TEMPLATES: Record<string, SmsTemplateDefinition> = {
  ...
  HOT_DEAL_SUBMISSION_DECISION: {
    code: 'HOT_DEAL_SUBMISSION_DECISION',
    name: 'Merchant Deal Submission Decision',
    category: 'ORDERS',
    description: 'Notifies merchant of admin approval, returned status, or rejection on submitted deal.',
    requiredVariables: ['deal_title', 'decision'],
    templateEn: 'LUMO Update: Your deal submission for "{{deal_title}}" has been {{decision}}.',
    templateSw: 'Taarifa ya LUMO: Uwasilishaji wa dili lako la "{{deal_title}}" {{decision}}.',
  },
  REFERRAL_CONFIRMED: {
    code: 'REFERRAL_CONFIRMED',
    name: 'Customer Referral Confirmed',
    category: 'ORDERS',
    description: 'Sent when customer successfully claims or converts through partner link.',
    requiredVariables: ['deal_title', 'customer_name'],
    templateEn: 'LUMO Alert: New referral confirmed for "{{deal_title}}"! Customer: {{customer_name}}.',
    templateSw: 'Taarifa ya LUMO: Rufaa mpya imethibitishwa kwa "{{deal_title}}"! Mteja: {{customer_name}}.',
  },
  DISPUTE_UPDATED: {
    code: 'DISPUTE_UPDATED',
    name: 'Dispute Case Progress Update',
    category: 'COMPLIANCE',
    description: 'Sent to merchant and partner when dispute status progresses.',
    requiredVariables: ['dispute_id', 'status'],
    templateEn: 'LUMO Dispute #{{dispute_id}}: Case status is now {{status}}. Please check your portal for updates.',
    templateSw: 'Mgogoro wa LUMO #{{dispute_id}}: Hali ya shauri sasa ni {{status}}. Tafadhali angalia akaunti yako.',
  },
  MERCHANT_REPORTED_REWARD_PAYMENT: {
    code: 'MERCHANT_REPORTED_REWARD_PAYMENT',
    name: 'Merchant Reported Reward Payment',
    category: 'FINANCIAL',
    description: 'Merchant reports direct reward payment to partner. Pending partner confirmation.',
    requiredVariables: ['merchant_name', 'amount', 'deal_title'],
    templateEn: 'Merchant {{merchant_name}} reported reward payment of TZS {{amount}} for deal "{{deal_title}}". Pending partner confirmation.',
    templateSw: 'Muuzaji {{merchant_name}} ameripoti malipo ya mgao wa TZS {{amount}} kwa dili "{{deal_title}}". Inasubiri uthibitisho wa mshirika.',
  },
  PARTNER_CONFIRMED_REWARD_RECEIPT: {
    code: 'PARTNER_CONFIRMED_REWARD_RECEIPT',
    name: 'Partner Confirmed Reward Receipt',
    category: 'FINANCIAL',
    description: 'Partner confirms receipt of direct reward from merchant. Dispute window closed.',
    requiredVariables: ['partner_name', 'amount', 'deal_title'],
    templateEn: 'Partner {{partner_name}} confirmed receipt of TZS {{amount}} reward for deal "{{deal_title}}". Transaction completed.',
    templateSw: 'Mshirika {{partner_name}} amethibitisha kupokea mgao wa TZS {{amount}} kwa dili "{{deal_title}}". Muamala umekamilika.',
  },
}
