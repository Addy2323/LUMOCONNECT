...
interface BusinessToastContextType {
  showToast: (type: ToastType, title: string, description?: string) => void
}

const BusinessToastContext = createContext<BusinessToastContextType | undefined>(undefined)

export function BusinessToastProvider({ children }: { children: React.ReactNode }) {
  ...
  return (
    <BusinessToastContext.Provider value={{ showToast }}>
      {children}
    ...
    </BusinessToastContext.Provider>
  )
}

export function useBusinessToast() {
  const context = useContext(BusinessToastContext)
  if (!context) {
    throw new Error('useBusinessToast must be used within a BusinessToastProvider')
  }
  return context
}
