...
export function DealRoomsTab() {
  ...
  const [selectedRoom, setSelectedRoom] = useState<DealRoomSession>(rooms[0])
  const [counterOfferAmount, setCounterOfferAmount] = useState(1500000)
  const [chatMessage, setChatMessage] = useState('')
  const [chatHistory, setChatHistory] = useState([
    {
      sender: 'Baraka Distribution Co.',
      time: 'Today, 10:15 AM',
      text: 'We have reviewed the regional distribution exclusivity terms for Lake Zone. We can commit to a 100-unit minimum initial purchase order.',
    },
    {
      sender: 'Alex Mushi (Kijani Solar)',
      time: 'Today, 11:30 AM',
      text: 'Terms accepted. We have attached the standard digital distribution SLA with milestone payouts on delivery inspection.',
    },
  ])
        ...
        <div className="lg:col-span-8 p-4 sm:p-5 rounded-3xl border border-slate-200 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-800/40 space-y-4">
          ...
          <div className="p-3.5 bg-white dark:bg-slate-900 rounded-2xl border text-xs space-y-2">
            <div className="flex items-center justify-between">
              <span className="font-bold text-slate-900 dark:text-white">Milestone Deliverable Summary:</span>
              <span className={`text-[10px] font-bold px-2 py-0.5 rounded ${
                selectedRoom.contractSigned ? 'bg-emerald-100 text-emerald-800' : 'bg-amber-100 text-amber-800'
              }`}>
                {selectedRoom.contractSigned ? '✓ Digital Agreement Signed' : 'Pending Signature'}
              </span>
            </div>
          ...
          <div className="space-y-2">
            <h4 className="text-xs font-extrabold uppercase tracking-wider text-slate-400">
              Deal Room Communication & Audit History
            </h4>
            ...
            <div className="flex gap-2">
              <input
                type="text"
                placeholder="Type negotiation terms, clarifications, or counteroffer..."
                value={chatMessage}
            ...
            <div className="flex gap-2">
              {!selectedRoom.contractSigned && (
                ...
                >
                  ...
                  <span>Countersign Digital Agreement</span>
                </button>
              )}
            </div>
