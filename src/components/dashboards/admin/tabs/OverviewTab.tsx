...
export function OverviewTab({ adminName, onOpenReviewQueue, onNavigateTab }: OverviewTabProps) {
  ...
  const handleExportAdminReport = () => {
    try {
      ...
      const fileName = `lumo-admin-executive-kpis-${timeRange.toLowerCase()}-${dateStr}.csv`

      const csvRows = [
        ['"LUMO COMMERCIAL PLATFORM - EXECUTIVE KPI & PERFORMANCE REPORT"'],
        [`"Generated By"`, `"${adminName}"`],
        [`"Generated At"`, `"${now.toLocaleString('en-GB')}"`],
        [`"Period Window"`, `"${timeRange}"`],
        [`"Region Filter"`, `"${regionFilter}"`],
        [`"Opportunity Filter"`, `"${opportunityTypeFilter}"`],
        [''],
        ['"EXECUTIVE KPI METRICS"', '"VALUE"'],
        ...
        ['"Pending Partner Payouts"', pendingPayouts],
        ['"Flagged Fraud & Risk Alerts"', flaggedFraud],
      ...
      showToast('success', 'Executive Report Exported', `CSV statement downloaded: ${fileName}`)
    } catch (err) {
    ...
    <div className="space-y-5 sm:space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-5">
        ...
        <div className="lg:col-span-8 bg-white dark:bg-slate-900 border border-[#E2E8F0] dark:border-slate-800 rounded-3xl p-5 sm:p-6 shadow-xs space-y-4">
          ...
          {(() => {
            const maxTx = Math.max(0, ...chartData.map((d) => d.txValue || 0))
            const maxUsers = Math.max(0, ...chartData.map((d) => d.activeUsers || 0))
            const hasActivity = maxTx > 0 || maxUsers > 0
                ...
                {!hasActivity && (
                  <div className="absolute inset-0 flex flex-col items-center justify-center bg-white/70 dark:bg-slate-900/70 backdrop-blur-[1px] pointer-events-none z-10 rounded-2xl">
                    <p className="text-xs font-bold text-slate-600 dark:text-slate-300">
                      No marketplace activity recorded for this period.
                    </p>
                    <p className="text-[11px] text-slate-400 dark:text-slate-500 mt-0.5">
                      Live transactions and user sessions will appear here automatically.
                    </p>
                  </div>
                )}
                <ResponsiveContainer width="100%" height="100%">
                  <ComposedChart data={chartData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                    ...
                    <YAxis
                      ...
                      axisLine={false}
                      tickLine={false}
                      domain={[0, maxTx > 0 ? 'auto' : 5]}
                      tick={{ fill: '#94A3B8', fontSize: 11 }}
                      tickFormatter={(val) => (val === 0 ? '0' : `TZS ${val}M`)}
                    />
                    <YAxis
                      yAxisId="right"
                      orientation="right"
                    ...
                    <Tooltip
                      ...
                      formatter={(val: any, name: any) => [
                        name.includes('Transaction') ? `TZS ${Number(val) * 1000000} TZS` : `${val} Users`,
                        name,
                      ]}
                    />
                    <Area
