...
import { TANZANIA_OPPORTUNITY_CATEGORIES, TANZANIA_REGIONS, getLocalizedCategoryLabel, getLocalizedSubcategoryLabel } from '@/modules/deals/taxonomy'
    ...
    <div className="bg-transparent sm:bg-white dark:sm:bg-slate-900 sm:border sm:border-[#E2E8F0] dark:sm:border-slate-800 rounded-2xl p-0 sm:p-4 mb-6 shadow-none sm:shadow-2xs">
      ...
      <div className="space-y-3 lg:hidden">
        ...
        <div className="relative">
          ...
          <input
            ...
            placeholder={locale === 'sw' ? 'Tafuta fursa, biashara au bidhaa' : 'Search deals, businesses or products'}
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          >
            ...
            <span>{locale === 'sw' ? 'Vichujio' : 'Filters'}</span>
            {activeFilterCount > 0 && (
          ...
          <div className="relative">
            ...
            >
              <option value="newest">{locale === 'sw' ? 'Panga: Mpya Zaidi' : 'Sort: Newest'}</option>
              <option value="recommended">{locale === 'sw' ? 'Panga: Zilizopendekezwa' : 'Sort: Recommended'}</option>
              <option value="highest_reward">{locale === 'sw' ? 'Panga: Zawadi ya Juu Zaidi' : 'Sort: Highest Reward'}</option>
            </select>
            ...
            >
              {CATEGORIES.map((cat) => (
                <option key={cat.value} value={cat.value}>
                  {cat.value === 'ALL'
                    ? (locale === 'sw' ? 'Makundi Yote' : 'All Categories')
                    : (locale === 'sw' ? getLocalizedCategoryLabel(cat.value, locale) : cat.label)}
                </option>
              ))}
            </select>
            ...
            >
              {OPPORTUNITY_TYPES.map((type) => (
                <option key={type.value} value={type.value}>
                  {type.value === 'ALL' ? (locale === 'sw' ? 'Aina Zote' : 'All Types') : t(type.label)}
                </option>
              ))}
            </select>
            <ChevronDown className="absolute right-3 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400 pointer-events-none" />
            ...
            >
              {REGIONS.map((r) => (
                <option key={r.value} value={r.value}>
                  {r.value === 'ALL' ? t('All Regions') : t(r.label)}
                </option>
              ))}
            </select>
            <ChevronDown className="absolute right-3 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400 pointer-events-none" />
      ...
      {showAdvanced && (
        <div className="mt-3 pt-3 border-t border-slate-100 dark:border-slate-800 space-y-3 bg-white dark:bg-slate-900 rounded-xl p-3 sm:p-0">
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            <div>
              <label className="block text-[11px] font-bold text-[#64748B] mb-1">{t('Category')}</label>
              <select
                value={selectedCategory}
                onChange={(e) => onCategoryChange(e.target.value)}
              ...
              >
                {CATEGORIES.map((c) => (
                  <option key={c.value} value={c.value}>
                    {c.value === 'ALL'
                      ? (locale === 'sw' ? 'Makundi Yote' : 'All Categories')
                      : (locale === 'sw' ? getLocalizedCategoryLabel(c.value, locale) : c.label)}
                  </option>
                ))}
              </select>
            </div>
            ...
            <div>
              <label className="block text-[11px] font-bold text-[#64748B] mb-1">{locale === 'sw' ? 'Aina' : 'Type'}</label>
              <select
                value={selectedType}
                onChange={(e) => onTypeChange(e.target.value)}
                className="w-full py-2 px-3 text-xs border rounded-xl bg-slate-50 dark:bg-slate-800"
              >
                {OPPORTUNITY_TYPES.map((tItem) => (
                  <option key={tItem.value} value={tItem.value}>
                    {tItem.value === 'ALL' ? (locale === 'sw' ? 'Aina Zote' : 'All Types') : t(tItem.label)}
                  </option>
                ))}
              </select>
            </div>
            ...
            <div>
              <label className="block text-[11px] font-bold text-[#64748B] mb-1">{t('Region')}</label>
              <select
                value={selectedRegion}
                onChange={(e) => onRegionChange(e.target.value)}
                className="w-full py-2 px-3 text-xs border rounded-xl bg-slate-50 dark:bg-slate-800"
      ...
      {activeFilterCount > 0 && (
        <div className="flex flex-wrap items-center gap-2 mt-3 pt-2 text-xs">
          <span className="text-[#64748B] font-semibold mr-1">{locale === 'sw' ? 'Inayotumika:' : 'Active:'}</span>
          ...
          {selectedCategory !== 'ALL' && (
            <span className="inline-flex items-center gap-1.5 px-2 py-0.5 rounded-lg bg-slate-100 dark:bg-slate-800 text-slate-800 dark:text-white text-[11px]">
              <span>{locale === 'sw' ? getLocalizedCategoryLabel(selectedCategory, locale) : selectedCategory}</span>
          ...
          {selectedRegion !== 'ALL' && (
            <span className="inline-flex items-center gap-1.5 px-2 py-0.5 rounded-lg bg-slate-100 dark:bg-slate-800 text-slate-800 dark:text-white text-[11px]">
              <MapPinLabel region={t(selectedRegion)} />
              <button onClick={() => onRegionChange('ALL')} className="hover:text-[#FF6A00]" aria-label={locale === 'sw' ? 'Futa kichujio cha mkoa' : 'Clear region filter'}>
                <X className="w-3 h-3" />
              </button>
            </span>
          )}
